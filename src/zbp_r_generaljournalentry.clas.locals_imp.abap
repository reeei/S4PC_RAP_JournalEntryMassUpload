...
*            明細項目
             ...
             financialtransactiontype      type zr_generaljournalentryitem-financialtransactiontype,
             taxcode                       TYPE zr_generaljournalentryitem-taxcode,
             partnercompany                type zr_generaljournalentryitem-partnercompany,
             ...
             withholdingtaxbaseamount      TYPE zr_generaljournalentryitem-withholdingtaxbaseamount,
             withholdingtaxamount          TYPE zr_generaljournalentryitem-withholdingtaxamount,
...
*      ls_entry-%param-exchangerate = ls_header->exchangerate.
      ...
      LOOP AT item REFERENCE INTO DATA(ls_item) USING KEY entity WHERE uuid = ls_header->uuid.
        ...
        CLEAR ls_glitem.
*       AR明細判定
        IF ls_item->%data-customer IS NOT INITIAL.
*          WHEN 'AR'.
          CLEAR ls_aritem.
          ...
          ls_aritem-assignmentreference = ls_item->%data-assignmentreference.
...
**********************************************************************
          ...
          ls_amount-exchangerate = ls_header->exchangerate.
...
*       AP明細判定
        ELSEIF ls_item->%data-supplier IS NOT INITIAL.
*        WHEN 'AP'.
          CLEAR ls_apitem.
          ...
          ls_apitem-assignmentreference = ls_item->%data-assignmentreference.
...
**********************************************************************
          IF ls_item->%data-withholdingtaxtype IS NOT INITIAL AND
            ...
            IF ls_item->%data-withholdingtaxbaseamount IS NOT INITIAL.
              ls_holdingtaxitem-whldgtaxbaseisenteredmanually = 'X'.
            ENDIF.
            IF ls_item->%data-withholdingtaxamount IS NOT INITIAL.
...
*              ls_amount-taxamount = '10.21-'.
            IF ls_item->%data-withholdingtaxbaseamount IS NOT INITIAL.
              ls_amount-taxbaseamount = ls_item->%data-withholdingtaxbaseamount.
              ls_amount-cashdiscountbaseamount = ls_item->%data-withholdingtaxbaseamount.
            ENDIF.
            IF ls_item->%data-withholdingtaxamount IS NOT INITIAL.
              ls_amount-journalentryitemamount = ls_item->%data-withholdingtaxamount.
              ls_amount-taxamount = ls_item->%data-withholdingtaxamount.
            ENDIF.
...
*          WHEN 'GL'.
          ...
          ls_glitem-financialtransactiontype = ls_item->%data-financialtransactiontype.
          ls_glitem-taxcode       = ls_item->%data-taxcode.
          ls_glitem-partnercompany = ls_item->%data-partnercompany.
          ...
          IF ls_item->%data-taxcode IS NOT INITIAL.
            CLEAR ls_taxitem.
*            ls_taxitem-glaccountlineitem = '3'.
            ls_taxitem-glaccountlineitem = |9{ ls_item->%data-referencedocumentitem }|.
...
***********************************************************************
          ...
          CLEAR ls_copa.
          ls_copa-customer = |{ ls_item->%data-customer_copa alpha = IN }|.
          ...
          ls_copa-salesorder = |{ ls_item->%data-salesorder_copa alpha = IN }|.
          ls_copa-salesorderitem = |{ ls_item->%data-salesorderitem_copa alpha = IN }|.
          ls_copa-plant = ls_item->%data-plant_copa.
          ls_copa-costcenter = |{ ls_item->%data-costcenter_copa alpha = IN }|.
          ls_copa-profitcenter = |{ ls_item->%data-profitcenter_copa alpha = IN }|.
...
*       endif.
  ...
  METHOD fileupload.
    ...
    DATA(lo_selection_pattern) = xco_cp_xlsx_selection=>pattern_builder->simple_from_to(
                                   ...
                                   )->to_column( xco_cp_xlsx=>coordinate->for_alphabetic_value( 'BV' )
    ...
    sort lt_journalentry by companycode
                            documentreferenceid
                            ledgergroup
...
*       明細単位処理
        ...
        ls_journalentryitem_c_target-taxamount = convert_price( i_price    = ls_journalentryitem-taxamount
                                                                i_currency = ls_journalentryitem-currencycode ).
        ls_journalentryitem_c_target-withholdingtaxbaseamount = convert_price( i_price    = ls_journalentryitem-withholdingtaxbaseamount
                                                                               i_currency = ls_journalentryitem-currencycode ).
        ls_journalentryitem_c_target-withholdingtaxamount = convert_price( i_price    = ls_journalentryitem-withholdingtaxamount
                                                                           i_currency = ls_journalentryitem-currencycode ).
...
*    DATA lt_journalentryitem type standard table of ty_journalentry_item with default key.
    ...
    lt_journalentry = VALUE #( (  companycode = '会社コード'
                                  ...
                                  taxamount = '税額'
                                  currencycode = '取引通貨'
                                  financialtransactiontype = '取引タイプ'
                                  taxcode = '税コード'
                                  partnercompany = '取引先コード'
...
* cl_abap_behavior_saver_failedを参照することでfailed構造が利用可能、かつエラー出力可能に
